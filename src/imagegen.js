...
/**
 * Generates token banner — tries Gemini first, then Pollinations, then SVG fallback
 */
export async function generateTokenImage(imagePrompt, symbol, tokenName) {
  ...
  // --- Option 1: Google Gemini Image Generation (free API key required) ---
  if (process.env.GEMINI_API_KEY) {
    try {
      ...
      if (imagePart?.inlineData?.data) {
        ...
        console.log(`✅ Image generated via Gemini (${Math.round(buffer.length / 1024)}KB)`);
        return tmpPath;
      }
    } catch (err) {
    ...
    console.log(`   ⚠️  No GEMINI_API_KEY set — skipping Gemini`);
  }

  // --- Option 2: Pollinations.ai (free, no key, but sometimes down) ---
  ...
  for (let i = 0; i < pollinationsUrls.length; i++) {
    try {
      console.log(`   Trying Pollinations (attempt ${i + 1})...`);
      if (i > 0) await sleep(5000);

      const response = await axios.get(pollinationsUrls[i], {
      ...
      if (response.data.length > 10000 && contentType.includes("image/")) {
        ...
        console.log(`✅ Image generated via Pollinations (${Math.round(response.data.length / 1024)}KB)`);
        return tmpPath;
      }
    } catch (err) {
      console.log(`   ❌ Pollinations attempt ${i + 1} failed: ${err.message}`);
    }
  }

  // --- Option 3: Branded SVG fallback (always works, token-specific) ---
...
function generateSVGFallback(symbol, tokenName) {
  ...
  <rect width="512" height="512" fill="url(#glow)" rx="40"/>
  ...
  <text x="256" y="210" font-family="Arial Black, Impact, sans-serif" font-size="68" font-weight="900"
    ...
    style="filter:drop-shadow(2px 2px 4px rgba(0,0,0,0.5))">$${symbol.slice(0, 6)}</text>
  ...
  <text x="256" y="430" font-family="Arial, sans-serif" font-size="22"
    fill="rgba(255,255,255,0.75)" text-anchor="middle">🚀 Powered by pump.fun</text>
  <text x="256" y="475" font-family="Arial, sans-serif" font-size="18"
    fill="rgba(255,255,255,0.5)" text-anchor="middle">via @clawpumptech</text>
</svg>`;
